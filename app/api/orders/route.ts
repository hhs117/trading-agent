...
import { getStoreById, isDatabaseConfigured, listOrdersFromDb } from "@/lib/server/database";
...
export async function GET(request: Request) {
  ...
  const orders = await listOrdersFromDb(storeId);
  return NextResponse.json({ ok: true, database: true, orders });
}
