...
import { dryRunShopeePublish, validateShopeeProductDraft } from "@/lib/api/shopee";
import type { ShopeeListingDraft } from "@/lib/server/shopee";
...
const sampleDraft = {
  region: "TH",
  title: "Sample Shopee listing draft",
  description: "Local validation draft for future Shopee publishing.",
  categoryId: null,
  brand: null,
  price: 199,
  currency: "THB",
  ...
  attributes: [],
  logistics: [],
  variationSkus: [{ sku: "sample-sku", price: 199, stock: 10 }],
  sourceUrl: null,
};

export default function ShopeePage() {
  return (
    <Suspense fallback={<div className="text-[13px] text-apple-gray-300">Loading Shopee workspace...</div>}>
      <ShopeePageInner />
    </Suspense>
  );
}

function ShopeePageInner() {
  const searchParams = useSearchParams();
  const productId = searchParams.get("productId")?.trim() || "";
  const [status, setStatus] = useState<ShopeeStatus | null>(null);
  const [issues, setIssues] = useState<ValidationIssue[]>([]);
  const [message, setMessage] = useState("");
  const [productDraft, setProductDraft] = useState<ShopeeListingDraft | null>(null);
  ...
  const readyItems = useMemo(
    () => [
      ...
      { label: "Shopee app credentials configured", done: Boolean(status?.configured) },
      { label: "OAuth callback and token refresh", done: false },
      { label: "Live item upload", done: false },
    ],
    [status?.configured]
  );

  async function loadStatus() {
    const response = await fetch("/api/shopee/status", { cache: "no-store" });
    if (!response.ok) return;
    const body = (await response.json()) as { shopee: ShopeeStatus };
    setStatus(body.shopee);
  }
  ...
  async function dryRunPublish() {
    ...
    const response = await fetch("/api/shopee/publish", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ draft: { ...sampleDraft, categoryId: 1, weightKg: 0.3 }, dryRun: true }),
    });
  ...
  const validateProductDraft = useCallback(async (nextProductId = productId) => {
    ...
    setMessage("");
    const body = await validateShopeeProductDraft({ productId: nextProductId });
  ...
  async function dryRunProductDraft() {
    ...
    setMessage("");
    const body = await dryRunShopeePublish(productDraft);
    setIssues(body.issues ?? []);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        icon={Store}
        title="Shopee integration groundwork"
        badge={status?.mode === "live" ? "Live config" : "Mock mode"}
        description="Prepare store authorization, listing validation, and one-click publish flow before the Shopee shop is ready."
      />

      {productId ? (
        <SectionCard
          title="Product Shopee draft"
          description="Generated from the selected product. Complete the missing category, attributes, logistics, and weight before live publishing."
          action={
            <Link
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[420px_1fr]">
        <SectionCard title="Connection status" description="Current environment readiness for Shopee Open Platform.">
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="secondary" icon={ClipboardCheck} onClick={validateDraft}>
                Validate sample
              </Button>
              <Button type="button" icon={Rocket} onClick={dryRunPublish}>
                Dry-run publish
              </Button>
            </div>
        ...
        <SectionCard title="Implementation checklist" description="What is already in place and what remains after shop approval.">
          <div className="space-y-3">
            {readyItems.map((item) => {
